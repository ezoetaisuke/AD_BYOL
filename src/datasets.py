...
class AudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 固定長チェック：
        #   - cfg['audio']['expect_same_length']=True の場合のみ厳密に一致を要求
        #   - 長さが違うと特徴量の shape（特に T）が変わり、バッチ化（collate）が破綻しやすい
        if len(y) != self.ref_len and self.cfg["audio"]["expect_same_length"]:
